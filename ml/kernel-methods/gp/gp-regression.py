...
def get_predictions2(xt, tt, cni, betai, x, sigma):
	...
	c = get_gp_covar(x, None, None, sigma) + betai
...
def plot_samples_posterior(xgt, tgt,
	...
	result = np.array([get_predictions(xgt, tgt, cni, betai, sigma, xx) for xx in x_test]).reshape(-1,2)
...
plot_samples_posterior(xgt, tgt, betai, x_test, sigma,          plot_posteriors, plot_mean)
plot_samples_posterior(xgt, tgt, betai, x_test, math.sqrt(1.0), plot_posteriors, plot_mean)
plot_samples_posterior(xgt, tgt, betai, x_test, math.sqrt(0.5), plot_posteriors, plot_mean)
#plot_samples_posterior(xgt, tgt, betai, x_test, math.sqrt(0.25), plot_posteriors, plot_mean)
#plot_samples_posterior(xgt, tgt, betai, x_test, math.sqrt(0.1), plot_posteriors, plot_mean)
plot_samples_posterior(xgt, tgt, betai, x_test, math.sqrt(0.01), plot_posteriors, plot_mean)
